...
# Configuración
# ============================================
BACKEND_URL = os.getenv("TEST_BACKEND_URL", "https://spmsystem2-0.onrender.com")
FRONTEND_URL = os.getenv("TEST_FRONTEND_URL", "https://spmv2-0.vercel.app")

# Credenciales desde variables de entorno (seguridad)
ADMIN_EMAIL = os.getenv("TEST_ADMIN_EMAIL", "admin@spm.local")
ADMIN_PASSWORD = os.getenv("TEST_ADMIN_PASSWORD")

# Validar que las credenciales estén configuradas
if not ADMIN_PASSWORD:
    print("\033[91m✗ ERROR: TEST_ADMIN_PASSWORD no configurada en variables de entorno\033[0m")
    print(
        "\033[93m⚠ Por favor ejecuta: export TEST_ADMIN_PASSWORD='tu_password' (Linux/Mac)\033[0m"
    )
    print("\033[93m⚠ O en Windows: set TEST_ADMIN_PASSWORD=tu_password\033[0m")
    sys.exit(1)
...
class ProductionTests:
    ...
    def test_csrf_token(self):
        ...
        try:
            response = self.session.get(urljoin(BACKEND_URL, "/api/auth/csrf"), timeout=10)
            print_info(f"Status Code: {response.status_code}")
    ...
    def test_login(self):
        ...
        try:
            ...
            response = self.session.post(
                urljoin(BACKEND_URL, "/api/auth/login"), json=data, headers=headers, timeout=10
            )
            print_info(f"Status Code: {response.status_code}")
    ...
    def test_cors(self):
        ...
        try:
            ...
            response = requests.options(
                urljoin(BACKEND_URL, "/api/materials"), headers=headers, timeout=10
            )
    ...
    def test_database(self):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                print_info(
                    f"Materiales disponibles: {len(data) if isinstance(data, list) else 'N/A'}"
                )
                self.passed += 1
    ...
    def print_summary(self):
        ...
        if self.failed == 0:
            print(
                f"{Colors.GREEN}{Colors.BOLD}🎉 TODOS LOS TESTS PASARON - PRODUCCIÓN LISTA{Colors.RESET}\n"
            )
            return 0
        else:
            print(
                f"{Colors.RED}{Colors.BOLD}❌ ALGUNOS TESTS FALLARON - REVISAR LOGS{Colors.RESET}\n"
            )
            return 1
