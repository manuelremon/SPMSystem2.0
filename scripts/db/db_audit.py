...
def run_audit(db_path):
    ...
    for t in tables:
        ...
        for c in cols:
            ...
            if typ.upper() == "TEXT" and name.lower() in (
                "activo",
                "habilitado",
                "flag",
                "is_active",
    ...
    for t in tables:
        ...
        for fk in fks:
            ...
            try:
                orphans = query_all(
                    con,
                    f"SELECT {from_col} FROM {t} LEFT JOIN {ref_table} ON {t}.{from_col} = {ref_table}.{to_col} WHERE {ref_table}.{to_col} IS NULL AND {t}.{from_col} IS NOT NULL LIMIT 50",
                )
                if orphans:
                    dq[t].setdefault("orphans", []).append(
                        {"fk": f"{from_col}->{ref_table}.{to_col}", "rows": [o[0] for o in orphans]}
                    )
            except Exception:
                continue
        ...
        if t == "usuarios":
            dups = query_all(
                con, "SELECT mail, COUNT(*) FROM usuarios GROUP BY mail HAVING COUNT(*) > 1"
            )
            if dups:
        ...
        if t == "almacenes":
            dups = query_all(
                con,
                "SELECT codigo, centro, COUNT(*) FROM almacenes GROUP BY codigo, centro HAVING COUNT(*) > 1",
            )
            if dups:
        ...
        for c in cols:
            if c[1].lower() in ("mail", "email"):
                ...
                bad = query_all(
                    con,
                    f"SELECT {c[1]} FROM {t} WHERE {c[1]} IS NOT NULL AND (instr({c[1]}, '@') = 0 OR instr({c[1]}, '.') = 0) LIMIT 10",
                )
                if bad:
            ...
            if c[1].lower() in ("telefono", "phone"):
                ...
                bad = query_all(
                    con,
                    f"SELECT {c[1]} FROM {t} WHERE {c[1]} IS NOT NULL AND {c[1]} GLOB '*[^0-9 ()+-]*' LIMIT 10",
                )
                if bad:
    ...
    for t in tables:
        ...
        for fk in fks:
            ...
            if not has_index_on(con, t, [from_col]):
                idx_suggestions.append(
                    {
                        "table": t,
                        "column": from_col,
                        "sql": f"CREATE INDEX IF NOT EXISTS idx_{t}_{from_col} ON {t}({from_col});",
                        "reason": "FK column, improves JOINs and deletes.",
                    }
                )
                ...
                norm.append(
                    {
                        "table": t,
                        "column": c[1],
                        "suggest": f"Consider CHECK constraint for allowed values on {t}.{c[1]}",
                    }
                )
    ...
    with open(os.path.join(outdir, "seed_suspects.sql"), "w", encoding="utf-8") as f:
        for s in seeds:
            f.write(f"{s}\n")
    print(
        f"Audit complete. Outputs written to {outdir}/\n  - report.md\n  - findings.json\n  - fixes.sql\n  - indexes.sql\n  - seed_suspects.sql\n  - erd.txt\n"
    )
