...
def _seed_user(db, u="alice", pw="secret123"):
    db.execute(
        "INSERT OR REPLACE INTO users(username,password_hash,email,display_name) VALUES(?,?,?,?)",
...
def test_update_profile_and_change_password():
    ...
    rv = client.put(
        ...
        json={"email": "new@x.com", "display_name": "A"},
    )
    assert rv.status_code == 200
    # wrong current
    rv = client.put(
        "/api/auth/password",
        headers={"Cookie": cookie},
        json={"current": "bad", "new": "longnewpw"},
    )
    ...
    rv = client.put(
        "/api/auth/password",
        headers={"Cookie": cookie},
        json={"current": "secret123", "new": "longnewpw"},
    )
    assert rv.status_code == 200
